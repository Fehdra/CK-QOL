...
{
    /// <summary>
    ///     Represents the "Eatable Binding" feature in the game, allowing players to quickly consume a configured or next available eatable item from their inventory.
    ///     This feature provides a key binding to trigger eating actions and automatically manages the inventory to locate and consume the eatable item efficiently.
    ///     
    ...
    ///             which determines the preferred slot to search for an eatable item (<see cref="EquipmentSlotIndex"/>).</description>
    ///         </item>
    ///         <item>
    ///             <description>Defines key bindings for quick eating, allowing users to set a custom key to trigger the eatable action (<see cref="ApplyKeyBinds"/> method).</description>
    ///         </item>
    ///         <item>
    ///             <description>Executes the logic to find, equip, and consume an eatable item from the inventory, 
    ///             ensuring efficient eating actions during gameplay (<see cref="Execute"/> and <see cref="ConsumeEatable"/> methods).</description>
    ///         </item>
    ///         <item>
    ///             <description>Monitors for key input and manages the eating process, including resetting the inventory state after eating (<see cref="Update"/> method).</description>
    ///         </item>
    ///     </list>
    /// </summary>
    /// <remarks>
    ...
    ///     It provides an optimized mechanism for eating by leveraging game input handling and inventory management functionalities.
    /// </remarks>
    internal sealed class QuickEat : FeatureBase<QuickEat>
    {
        ...
        {
            RewiredExtensionModule.AddKeybind(KeyBindName, "Quick Eat", KeyboardKeyCode.G);
        }
        ...
        /// <returns>
        ///     <see langword="true" /> if an eatable item was found; otherwise, <see langword="false" />.
        /// </returns>
        ...
        /// <returns>
        ///     <see langword="true" /> if the object is Eatable; otherwise, <see langword="false" />.
        /// </returns>
